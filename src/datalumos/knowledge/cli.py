...
def add(
    ...
    source_type: str = typer.Option(
        "filesystem", "--type", "-t", help="Source type: filesystem or web"
    ),
) -> None:
    ...
    try:
        ...
        file_id = manager.upload_and_add_document(
            vector_store_id=vector_store_id, source=source, source_type=source_type
        )
...
def list() -> None:
    ...
    try:
        ...
        for file in files:
            table.add_row(file["id"], file["status"], str(file["created_at"]))
...
def delete_store(
    ...
    try:
        ...
        if success:
            console.print(
                f"✓ Vector store deleted successfully! Store ID: {vector_store_id}"
            )
        else:
