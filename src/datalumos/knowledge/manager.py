...
class KnowledgeManager:
    ...
    def __init__(self, openai_api_key: str | None = None) -> None:
        ...
        if not api_key:
            raise ValueError(
                "OpenAI API key is required. Provide it or set OPENAI_API_KEY env var."
            )
    ...
    def upload_and_add_document(
        ...
        if source_type == "filesystem":
            ...
            raise ValueError(
                f"Invalid source_type: {source_type}. Must be 'filesystem' or 'web'"
            )
    ...
    def create_knowledge_base(
        ...
        logger.info(
            f"Successfully created knowledge base with {len(file_ids)} documents"
        )
