...
from datalumos.agents.triage_agent import TriageAgent, TriageOutput, ColumnImportance
...
class TableAnalysisResults(BaseModel):
    ...
    column_analyses: list[ColumnAnalysisOutput] = Field(description="Detailed analysis for each column")
...
async def _triage_columns(
    ...
    logger.info(f"Column triage complete for {schema}.{table_name}. Column analyses: {column_analyses}")
...
def _save_cached_results(schema: str, table_name: str, results: TableAnalysisResults) -> None:
