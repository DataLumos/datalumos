...
class ColumnAnalyserAgent(Agent):
    ...
    def __init__(
        self, mcp_servers: list[MCPServerStdio], column_name: str, table_context: str
    ):
        tools = [WebSearchTool()]
...
class ColumnAnalysisOutput(BaseModel):
    ...
    valid_values: str = Field(description="Acceptable values or ranges for this column")
    quality_rules: str = Field(description="Data quality rules and validation criteria")
    sources: str = Field(
    ...
    table_description: str = Field(description="Description of the parent table")
